...
# The input name is not valid because it contains U+D7FF (arround
# the surrogate area).
#
#             api: decodename
...
echo 'from: xn--hb9b' >> expect.txt
echo 'result: success' >> expect.txt
echo 'to: ퟿' >> expect.txt
../common/test_decodename -conffile=idn2.conf -localcheckfile=  -idnconv -localconv -- 'xn--hb9b' > output.txt
