...
# Reigster U+DFFF as a delimiter.
#
#             api: encodename
#  local_encoding: U-escape
#            conf: map delimitermap
#            conf: delimiters dfff
#            from: \u{dfff}
...
echo 'delimiters dfff' >> idn2.conf
...
echo 'from: \u{dfff}' >> expect.txt
echo 'result: idn_resconf_loadfile() ERROR: invalid code point' >> expect.txt
echo 'to: ' >> expect.txt
../common/test_encodename -conffile=idn2.conf -localcheckfile=  -unicodeconv -map -localconv -- '\u{dfff}' > output.txt
