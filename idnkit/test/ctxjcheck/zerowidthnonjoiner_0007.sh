...
# The input name is U+0061 U+200C U+062F
# (Joining_Type:U + U+200C + Joining_Type:R).
#
#             api: encodename
...
echo 'from: a\u{200c}\u{62f}' >> expect.txt
echo 'result: CONTEXTJ check failed' >> expect.txt
echo 'to: ' >> expect.txt
../common/test_encodename -conffile=idn2.conf -localcheckfile=  -unicodeconv -ctxjcheck -localconv -- 'a\u{200c}\u{62f}' > output.txt
