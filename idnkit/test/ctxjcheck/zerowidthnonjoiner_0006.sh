...
# The input name is U+200C U+0628 (U+200C + Joining_Type:D).
#
#             api: encodename
...
echo 'from: \u{200c}\u{628}' >> expect.txt
echo 'result: CONTEXTJ check failed' >> expect.txt
echo 'to: ' >> expect.txt
../common/test_encodename -conffile=idn2.conf -localcheckfile=  -unicodeconv -ctxjcheck -localconv -- '\u{200c}\u{628}' > output.txt
