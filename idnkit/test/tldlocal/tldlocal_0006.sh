...
# Convert U+004A (J) to U+006A (J) if TLD is "jp".
# The input name has only one label. idnkit doesn't convert the label
# even if it matches TLD of 'tld-local'.
#
#             api: encodename
...
echo 'from: JP' >> expect.txt
echo 'result: success' >> expect.txt
echo 'to: JP' >> expect.txt
../common/test_encodename -conffile=idn2.conf -localcheckfile=  -unicodeconv -map -localconv -- 'JP' > output.txt
