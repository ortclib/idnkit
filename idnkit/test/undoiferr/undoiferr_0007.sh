...
#
# IDN_UNDOIFERR test.
# The input name is U+200C (CONTEXTJ code point) + U+0041 ("A").
#
#             api: encodename
...
echo 'from: \u{200c}A' >> expect.txt
echo 'result: success' >> expect.txt
echo 'to: \u{200c}a' >> expect.txt
../common/test_encodename -conffile=idn2.conf -localcheckfile=  -encode-regist -localconv -undoiferr -- '\u{200c}A' > output.txt
