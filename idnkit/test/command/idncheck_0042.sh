...
#
#             api: command
#         command: ../../tools/idncheck/idncheck -version 2>&1 | sed -e 's/:.*$//'
#          output: idncheck (idnkit) version
...
echo 'idncheck (idnkit) version' >> expect.txt
echo 'Unicode version' >> expect.txt
echo 'exitcode=0' >> expect.txt
rm -f input.txt
touch input.txt
cat input.txt | ../../tools/idncheck/idncheck -version 2>&1 | sed -e 's/:.*$//' > output.txt 2>&1
