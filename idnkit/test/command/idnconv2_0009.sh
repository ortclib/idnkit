...
# Test '-out' option without an argument.
#
#             api: command
#         command: ../../tools/idnconv/idnconv2 -c /dev/null -out
#          output: option '-out' requires an argument.
...
echo 'option '\''-out'\'' requires an argument.' >> expect.txt
echo 'try '\''idnconv2 -help'\'' for more information.' >> expect.txt
echo 'exitcode=1' >> expect.txt
rm -f input.txt
touch input.txt
cat input.txt | ../../tools/idnconv/idnconv2 -c /dev/null -out > output.txt 2>&1
