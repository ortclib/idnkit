...
#
#             api: command
#         command: ../../tools/idnconv/idnconv2 -version 2>&1 | sed -e 's/:.*$//'
#          output: idnconv (idnkit) version
...
echo 'idnconv (idnkit) version' >> expect.txt
echo 'Unicode version' >> expect.txt
echo 'exitcode=0' >> expect.txt
rm -f input.txt
touch input.txt
cat input.txt | ../../tools/idnconv/idnconv2 -version 2>&1 | sed -e 's/:.*$//' > output.txt 2>&1
