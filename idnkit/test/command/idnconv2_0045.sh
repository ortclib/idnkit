...
# Test '-whole' option with '-reverse'.
#
#             api: command
#  local_encoding: U-escape
#         command: ../../tools/idnconv/idnconv2 -c /dev/null -whole -skip prohcheck -reverse
#           input: xn--=== a ===-t1a
#          output: === \u{e0}a ===
#        exitcode: 0
...
echo '=== \u{e0}a ===' >> expect.txt
echo 'exitcode=0' >> expect.txt
rm -f input.txt
touch input.txt
echo 'xn--=== a ===-t1a' >> input.txt
cat input.txt | ../../tools/idnconv/idnconv2 -c /dev/null -whole -skip prohcheck -reverse > output.txt 2>&1
