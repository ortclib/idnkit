...
# NAME is not an valid IDN.
#
#             api: command
#  local_encoding: U-escape
#         command: ../../tools/idnconv/idnconv2 -c /dev/null
#           input: === \u{300} ===
#          output: conversion failed at line 1: combining character check failed
#        exitcode: 1
...
echo 'conversion failed at line 1: combining character check failed' >> expect.txt
echo 'exitcode=1' >> expect.txt
rm -f input.txt
touch input.txt
echo '=== \u{300} ===' >> input.txt
cat input.txt | ../../tools/idnconv/idnconv2 -c /dev/null > output.txt 2>&1
