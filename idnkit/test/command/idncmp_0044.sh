...
#
#             api: command
#         command: ../../tools/idncmp/idncmp -version 2>&1 | sed -e 's/:.*$//'
#          output: idncmp (idnkit) version
...
echo 'idncmp (idnkit) version' >> expect.txt
echo 'Unicode version' >> expect.txt
echo 'exitcode=0' >> expect.txt
rm -f input.txt
touch input.txt
cat input.txt | ../../tools/idncmp/idncmp -version 2>&1 | sed -e 's/:.*$//' > output.txt 2>&1
