...
# Specify both '-lookup' and '-registration' option.
#
#             api: command
#  local_encoding: U-escape
#         command: ../../tools/idnconv/idnconv2 -c /dev/null -lookup -registration
#           input: === \u{e0}.-.jp ===
#          output: conversion failed at line 1: hyphen check failed
#        exitcode: 1
...
echo 'conversion failed at line 1: hyphen check failed' >> expect.txt
echo 'exitcode=1' >> expect.txt
rm -f input.txt
touch input.txt
echo '=== \u{e0}.-.jp ===' >> input.txt
cat input.txt | ../../tools/idnconv/idnconv2 -c /dev/null -lookup -registration > output.txt 2>&1
