...
# The input name is Punycode U+05D0 (Bidiclass=R) + U+0061 (Bidiclass=L).
#
#             api: decodename
...
echo 'from: XN--A-ZHC' >> expect.txt
echo 'result: Bidi check failed' >> expect.txt
echo 'to: ' >> expect.txt
../common/test_decodename -conffile=idn2.conf -localcheckfile=  -decode-lookup -- 'XN--A-ZHC' > output.txt
