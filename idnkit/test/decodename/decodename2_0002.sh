...
#
# idn_res_decodename2() API test.
# Both the auxencoding and the local encoding is U-escape.
#
#             api: decodename2
#  local_encoding: U-escape
#     auxencoding: U-escape
#            conf: 
#            from: \u{78}\u{6e}--0ca
...
echo 'auxencoding: U-escape' >> expect.txt
echo 'from: \u{78}\u{6e}--0ca' >> expect.txt
echo 'result: success' >> expect.txt
echo 'to: \u{e0}' >> expect.txt
../common/test_decodename2 -conffile=idn2.conf -localcheckfile= -auxencoding=U-escape  -decode-regist -- '\u{78}\u{6e}--0ca' > output.txt
