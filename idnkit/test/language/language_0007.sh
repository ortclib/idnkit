#! /bin/sh
# Do not edit this file!
# this file is generated by 'generate_test.pl'.

#
# The current language test.
# Determine the the current language from the current locale.
# The environment variable LANG lacks an encoding name.
#
# On solaris, set LANG to 'ja_JP' causes an error; "couldn't set locale
# correctly".
#
#              os: Linux
#             api: language
#             env: LANG=ja_JP
#            conf: 
#  language_alias: 
#          expect: ja

################## setup ##################

unset LC_ALL
unset LC_CTYPE
unset LC_MESSAGES
unset LANG
unset IDN_LOCAL_CODESET
unset IDN_LOG_LEVEL

SRCDIR=`dirname $0`
SYSCONFDIR=${SYSCONFDIR-"/etc"}
IGNORE=false
export LANG; LANG=ja_JP

[ `uname -s` = Linux ] || IGNORE=true
rm -f idn2.conf
touch idn2.conf
echo '' >> idn2.conf

touch ${SYSCONFDIR}/idnlang.conf || IGNORE=true
rm -f ${SYSCONFDIR}/idnlang.conf
( echo '' >> ${SYSCONFDIR}/idnlang.conf )

rm -f localmap1
rm -f localmap2
rm -f localmap3
rm -f localmap4
rm -f localset
rm -f expect.txt
rm -f output.txt

################## test ##################

echo 'ja' >> expect.txt
../common/test_language -conffile=idn2.conf > output.txt

################## teardown ##################

cmp expect.txt output.txt > /dev/null 2>&1
RESULT=$?
[ X$IGNORE = Xtrue ] && RESULT=77
if [ X$QUIET != Xtrue ]; then
    if [ $RESULT -eq 0 ]; then
        echo "PASS: $0"
    elif [ $RESULT -eq 77 ]; then
        echo "SKIP: $0"
    else
        echo "FAIL: $0"
    fi
fi
[ $RESULT -eq 0 -o $RESULT -eq 77 ] || exit $RESULT
rm -f idn2.conf
rm -f ${SYSCONFDIR}/idnlang.conf
rm -f localmap1
rm -f localmap2
rm -f localmap3
rm -f localmap4
rm -f localset
rm -f expect.txt
rm -f output.txt
exit $RESULT
