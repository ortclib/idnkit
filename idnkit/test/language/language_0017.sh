...
# A test for language alias file.
# An syntax occurs; missing canonical name.
#
#             api: language
#            conf: language JAPANESE
#  language_alias: Japanese
#          expect: idn_resconf_initialize() ERROR: syntax error
...
rm -f idn2.conf
touch idn2.conf
echo 'language JAPANESE' >> idn2.conf

touch ${SYSCONFDIR}/idnlang.conf || IGNORE=true
rm -f ${SYSCONFDIR}/idnlang.conf
( echo 'Japanese' >> ${SYSCONFDIR}/idnlang.conf )
...
echo 'idn_resconf_initialize() ERROR: syntax error' >> expect.txt
