...
# The first character of the input name is U+00AC (General_Category=Sm).
#
#             api: encodename
...
echo 'from: \u{ac}' >> expect.txt
echo 'result: success' >> expect.txt
echo 'to: \u{ac}' >> expect.txt
../common/test_encodename -conffile=idn2.conf -localcheckfile=  -unicodeconv -combcheck -localconv -- '\u{ac}' > output.txt
