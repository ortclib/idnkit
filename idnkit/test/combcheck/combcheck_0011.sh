...
# The first character of the input name is U+00B2 (General_Category=No).
#
#             api: encodename
...
echo 'from: \u{b2}' >> expect.txt
echo 'result: success' >> expect.txt
echo 'to: \u{b2}' >> expect.txt
../common/test_encodename -conffile=idn2.conf -localcheckfile=  -unicodeconv -combcheck -localconv -- '\u{b2}' > output.txt
