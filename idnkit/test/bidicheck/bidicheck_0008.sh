...
# The input name is just U+202C (bidiclass: PDF).
#
#             api: encodename
...
echo 'from: \u{202c}' >> expect.txt
echo 'result: success' >> expect.txt
echo 'to: \u{202c}' >> expect.txt
../common/test_encodename -conffile=idn2.conf -localcheckfile=  -unicodeconv -bidicheck -localconv -- '\u{202c}' > output.txt
