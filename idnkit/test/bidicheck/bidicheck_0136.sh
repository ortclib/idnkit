...
# The input name is U+0030 U+00E0 + "." + U+05D0 (bidiclass: EN L . R).
#
#             api: encodename
#  local_encoding: U-escape
#            conf: 
#            from: xn--0-sfa.xn--4db
...
echo 'from: xn--0-sfa.xn--4db' >> expect.txt
echo 'result: Bidi check failed' >> expect.txt
echo 'to: ' >> expect.txt
../common/test_encodename -conffile=idn2.conf -localcheckfile=  -encode-lookup -- 'xn--0-sfa.xn--4db' > output.txt
