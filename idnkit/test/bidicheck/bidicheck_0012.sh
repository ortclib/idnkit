...
# The input name is just U+0600 (bidiclass: AN).
#
#             api: encodename
...
echo 'from: \u{600}' >> expect.txt
echo 'result: Bidi check failed' >> expect.txt
echo 'to: ' >> expect.txt
../common/test_encodename -conffile=idn2.conf -localcheckfile=  -unicodeconv -bidicheck -localconv -- '\u{600}' > output.txt
