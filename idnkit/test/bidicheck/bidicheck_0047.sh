...
# The input name is U+202C + "." + U+05D0 (bidiclass: PDF . R).
#
#             api: encodename
...
echo 'from: \u{202c}.\u{5d0}' >> expect.txt
echo 'result: Bidi check failed' >> expect.txt
echo 'to: ' >> expect.txt
../common/test_encodename -conffile=idn2.conf -localcheckfile=  -unicodeconv -bidicheck -localconv -- '\u{202c}.\u{5d0}' > output.txt
