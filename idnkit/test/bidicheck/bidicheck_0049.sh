...
# The input name is U+002B + "." + U+05D0 (bidiclass: ES . R).
#
#             api: encodename
...
echo 'from: +.\u{5d0}' >> expect.txt
echo 'result: Bidi check failed' >> expect.txt
echo 'to: ' >> expect.txt
../common/test_encodename -conffile=idn2.conf -localcheckfile=  -unicodeconv -bidicheck -localconv -- '+.\u{5d0}' > output.txt
