...
# The input name is just U+06F8 U+0661
# (EXTENDED ARABIC-INDIC DIGIT ONE and ARABIC-INDIC DIGIT EIGHT).
#
#             api: encodename
...
echo 'from: \u{6f8}\u{661}' >> expect.txt
echo 'result: CONTEXTO check failed' >> expect.txt
echo 'to: ' >> expect.txt
../common/test_encodename -conffile=idn2.conf -localcheckfile=  -unicodeconv -ctxocheck -localconv -- '\u{6f8}\u{661}' > output.txt
