...
# The input name is just U+0061 U+30FB. (U+0061 belongs to Common)
#
#             api: encodename
...
echo 'from: a\u{30fb}' >> expect.txt
echo 'result: CONTEXTO check failed' >> expect.txt
echo 'to: ' >> expect.txt
../common/test_encodename -conffile=idn2.conf -localcheckfile=  -unicodeconv -ctxocheck -localconv -- 'a\u{30fb}' > output.txt
