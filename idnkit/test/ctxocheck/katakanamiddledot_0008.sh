...
# The input name is U+8A66 U+30FB. (U+8A66 belongs to Han)
#
#             api: encodename
...
echo 'from: \u{8a66}\u{30fb}' >> expect.txt
echo 'result: success' >> expect.txt
echo 'to: \u{8a66}\u{30fb}' >> expect.txt
../common/test_encodename -conffile=idn2.conf -localcheckfile=  -unicodeconv -ctxocheck -localconv -- '\u{8a66}\u{30fb}' > output.txt
