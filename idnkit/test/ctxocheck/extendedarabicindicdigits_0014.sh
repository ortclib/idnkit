...
# The input name is just U+06F6 U+0663
# (EXTENDED ARABIC-INDIC DIGIT THREE and ARABIC-INDIC DIGIT SIX).
#
#             api: encodename
...
echo 'from: \u{6f6}\u{663}' >> expect.txt
echo 'result: CONTEXTO check failed' >> expect.txt
echo 'to: ' >> expect.txt
../common/test_encodename -conffile=idn2.conf -localcheckfile=  -unicodeconv -ctxocheck -localconv -- '\u{6f6}\u{663}' > output.txt
