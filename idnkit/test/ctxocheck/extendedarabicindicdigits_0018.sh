...
# The input name is just U+06F2 U+0667
# (EXTENDED ARABIC-INDIC DIGIT SEVEN and ARABIC-INDIC DIGIT TWO).
#
#             api: encodename
...
echo 'from: \u{6f2}\u{667}' >> expect.txt
echo 'result: CONTEXTO check failed' >> expect.txt
echo 'to: ' >> expect.txt
../common/test_encodename -conffile=idn2.conf -localcheckfile=  -unicodeconv -ctxocheck -localconv -- '\u{6f2}\u{667}' > output.txt
