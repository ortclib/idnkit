...
# The input name is just U+0660 U+06F9
# (ARABIC-INDIC DIGIT ZERO and EXTENDED ARABIC-INDIC DIGIT NINE).
#
#             api: encodename
...
echo 'from: \u{660}\u{6f9}' >> expect.txt
echo 'result: CONTEXTO check failed' >> expect.txt
echo 'to: ' >> expect.txt
../common/test_encodename -conffile=idn2.conf -localcheckfile=  -unicodeconv -ctxocheck -localconv -- '\u{660}\u{6f9}' > output.txt
