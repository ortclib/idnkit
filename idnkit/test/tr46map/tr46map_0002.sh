...
#
# TR #46 mapping test.
# The input name is U+00DF (LATIN SMALL LETTER SHARP S).
# It is converted to U+0073 U+0073 ("ss") in case of 'tr46-map-deviation'.
#
#             api: encodename
#  local_encoding: U-escape
#            conf: map tr46-map-deviation
#            from: \u{df}
...
rm -f idn2.conf
touch idn2.conf
echo 'map tr46-map-deviation' >> idn2.conf
...
echo 'from: \u{df}' >> expect.txt
echo 'result: success' >> expect.txt
echo 'to: ss' >> expect.txt
../common/test_encodename -conffile=idn2.conf -localcheckfile=  -unicodeconv -map -localconv -- '\u{df}' > output.txt
