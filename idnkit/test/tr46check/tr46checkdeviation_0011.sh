...
# The input name is U+FF21 (FULL WIDTH "A").
# TR #46 defines the code point is MAPPED.
#
#             api: decodename
...
echo 'from: \u{ff21}' >> expect.txt
echo 'result: TR #46 check failed' >> expect.txt
echo 'to: ' >> expect.txt
../common/test_decodename -conffile=idn2.conf -localcheckfile=  -unicodeconv -map -localconv -- '\u{ff21}' > output.txt
