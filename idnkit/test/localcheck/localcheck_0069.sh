...
#
# local check test.
# A code point in an input name is not valid.
#
#             api: decodename
...
echo 'from: \u{e0}\u{e1}\u{e2}' >> expect.txt
echo 'result: local check failed' >> expect.txt
echo 'to: ' >> expect.txt
../common/test_decodename -conffile=idn2.conf -localcheckfile=localset  -unicodeconv -localcheck -localconv -- '\u{e0}\u{e1}\u{e2}' > output.txt
