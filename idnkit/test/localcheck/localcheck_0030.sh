...
echo 'from: \u{e1}\u{e2}\u{e3}\u{e0}\u{e4}' >> expect.txt
echo 'result: local check failed' >> expect.txt
echo 'to: ' >> expect.txt
../common/test_encodename -conffile=idn2.conf -localcheckfile=localset  -unicodeconv -localcheck -localconv -- '\u{e1}\u{e2}\u{e3}\u{e0}\u{e4}' > output.txt
